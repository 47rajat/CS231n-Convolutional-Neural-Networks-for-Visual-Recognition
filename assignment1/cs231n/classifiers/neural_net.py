...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    grads['W2'] = A1.T.dot(dZ2)/X.shape[0] + 2*reg*W2
    grads['b2'] = np.sum(dZ2, axis=0)/X.shape[0]

    dZ1 = dZ2.dot(W2.T)
    dZ1 *= mask
    grads['W1'] = X.T.dot(dZ1)/X.shape[0] + 2*reg*W1
    grads['b1'] = np.sum(dZ1, axis=0)/X.shape[0]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
